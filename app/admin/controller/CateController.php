...
class CateController extends AdminCheckAuth
{
    ...
    public function addSubmit()
    {
    	if($this->request->isPost()){
    		$result = $this->validate(jt_get_param(), 'app\admin\validate\ArticleCate.add');
            if ($result !== true) {
                return jt_assign(0,$result);
            } else {
                \think\loader::model('ArticleCate')->cache('VAE_ARTICLE_CATE')->strict(false)->field(true)->insert(jt_get_param());
                return jt_assign();
            }
    	}
    }

...
class AdminController extends AdminCheckAuth
{
    ...
    public function getAdminList()
    {
    	...
    	return jt_assign_table(0,'',$admin);
    }
            ...
            if ($result !== true) {
				...
				try{
                	...
                	foreach ($param['group_id'] as $k => $v) {
                        ...
                        if($v == 1 and jt_get_login_admin('id') !== 1) {
				    ...
				    // 回滚事务
				    Db::rollback();
				    return jt_assign(0,'提交失败:'.$e->getMessage());
				}
                return jt_assign();
            }
    	}
    ...
    {
        return view('',['admin'=>jt_get_admin(jt_get_param('id'))]);
    }
    ...
    public function delete()
    {
        $id    = jt_get_param("id");
        if ($id == 1) {
            return jt_assign(0,"系统拥有者，无法删除！");
        }
        if (Db::name('Admin')->delete($id) !== false) {
            return jt_assign(1,"删除管理员成功！");
        } else {
